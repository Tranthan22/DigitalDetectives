...
#Core 1:
    while not check_data_transmit:
        ...
        receive_node = decode_data_receive()
        time.sleep(1)
        receive_node_2 = decode_data_receive()
        line = ser.readline()
        if line:
...
#Core 2:
    ...
    encode_data_transmit(NODE_SENSOR_1, pad_array(str(Check_data)))
...
#Core 4:
    ...
    #transmit_motor('m1', data_transmit_motor)
    encode_data_transmit(str(data_transmit_motor))
    time.sleep(2)
