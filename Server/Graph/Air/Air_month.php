<?php
    ...
    $monthData = array();
    $query = $pdo->prepare("SELECT DATE(date) AS date, AVG(node_air) AS average FROM $table_name WHERE DATE_FORMAT(date, '%Y-%m') = :month AND garden = :garden_id GROUP BY DATE(date)");
